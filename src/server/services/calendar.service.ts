...
// Errors fixed:
//   Error 1 (line ~573): attendees was string[] so `.email` doesn't exist → use `a` directly
//   Error 2 (line ~581): input.calendarType didn't exist on old CreateEventInput → now it does
//   Error 3 (line ~618): input.calendarType didn't exist on old UpdateEventInput → now it does
...
// export type RSVPStatusType = "tentative" | "needsAction" | "declined" | "accepted";
  ...
  await db
    ...
    .onConflictDoUpdate({
      ...
      set: {
        ...
        attendees: attendeeValues,
        status: (event.status as CalendarEvent["status"]) ?? "confirmed",
        // Only overwrite calendarType if explicitly provided (not the default fallback)
        // This prevents a background sync from resetting "Meetings" → "Work"
        updatedAt: new Date(),
      },
    });
}
