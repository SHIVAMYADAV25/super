...
/**
 * Search service — hybrid text + semantic search across Gmail + Calendar.
 *
 * Text search:  Corsair local DB → Gmail API fallback
 * Semantic:     pgvector cosine distance on stored email embeddings
 * Hybrid:       both in parallel, deduplicated, semantic first
 *
 * Embeddings are generated via getEmbedding() from llm-provider.ts
 * so the model (OpenAI / nemotron / gemini) can be swapped via env var.
 *
 * Pattern: tenantId (googleSub) → Corsair, userId (DB UUID) → DB queries
 */
...
export async function storeEmailEmbedding(
  ...
  try {
    ...
    `);
    logger.debug("Email embedding stored", { userId, gmailId, dim: EMBEDDING_DIM });
  } catch (err) {
    logger.warn("Failed to store email embedding", { userId, gmailId, error: String(err) });
  }
}