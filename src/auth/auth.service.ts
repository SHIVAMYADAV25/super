...
export async function getOrCreateUser(profile : GoogleProfile) {
    ...
    if(existing.length > 0){
        // Backfill googleSub for users created before this column existed,
        // or if it's somehow missing — webhook routing depends on it.
        if (existing[0]!.googleSub !== profile.sub) {
            await db
                .update(users)
                .set({ googleSub: profile.sub, updatedAt: new Date() })
                .where(eq(users.id, existing[0]!.id));
            return { ...existing[0]!, googleSub: profile.sub };
        }
        return existing[0]
    }
